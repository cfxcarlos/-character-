...
function setGender(gender)    
    ...
    TriggerEvent("fenixrand",currentArmour)
...
local barberShops = {
    ...
    [8] = { coords = vec3(1248.23,-207.27,100.1) },
    [9] = { coords = vec3(-796.66,326.0,243.38) },
    [10] = { coords = vec3(2517.34,-345.25,101.89) },
    [11] = { coords = vec3(1402.17,1154.13,114.38) }, 
    [12] = { coords = vec3(-330.89,1544.02,370.83) }, 
    [13] = { coords = vec3(3015.9,3058.22,115.0) }, 
    [14] = { coords = vec3(-1422.5,5128.63,66.05) }, 
    [15] = { coords = vec3(-1718.86,-133.33,85.7) }, --penha  
    [16] = { coords = vec3(2544.7,3432.49,74.29) }, -- cidade alta 
    [17] = { coords = vec3(2546.4,2417.49,53.85) }, --turano
    [18] = { coords = vec3(1894.16,6389.68,76.18) }, --vk  
    [19] = { coords = vec3(1780.0,-2316.07,151.78) }, -- caixa d agua  
    [20] = { coords = vec3(600.59,892.0,233.82) }, -- alemao 
    [21] = { coords = vec3(1928.89,6438.9,76.36) }, -- vk
    [22] = { coords = vec3(-1820.0,26.26,94.59) }, -- penha
    [23] = { coords = vec3(-2187.75,-190.37,61.33) }, -- rodo
    [24] = { coords = vec3(1558.95,-785.57,111.48) }, -- Rocinha
...
CreateThread(function()
	...
	for Number = 1,#barberShops do
        Tables[#Tables + 1] = { barberShops[Number]["coords"]["x"],barberShops[Number]["coords"]["y"],barberShops[Number]["coords"]["z"],2.0,"E","Barbearia","Pressione para abrir" }
	end
...
function drawChairMarkers()
    ...
    for k,v in ipairs(barberChairs[barberId]) do
        ...
        DrawMarker(1,x,y,z-1.9,0,0,0,0,0,0,0.75,0.75,1.0,45, 80, 233,70,0,1,1,1)
    end
end
    ...
    while nearestBarbershop do
        if not inMenu then
            ...
            if nearestChair then
                if IsDisabledControlJustPressed(0, 38) and GetEntityHealth(ped) > 101 and func.checkChair(barberId, chairIndex, nearestBarbershop.perm) then
