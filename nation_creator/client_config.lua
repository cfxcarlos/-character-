...
creatorCoords = vec3(566.14,-445.71,-69.66)
creatorHeading = 249.45

spawnCoords = vec3(-1039.38,-2740.68,13.87)
spawnHeading = 56.7
...
function initCreator()
    ...
    SetEntityHeading(ped, creatorHeading)
    setGender("male")
    TriggerEvent("reloadtattos", { tattoos = {}, overlay = 0 })
...
function getOverlays()
    ...
    elseif GetResourceState("nation_barbershop") == "started" then
        return exports["nation_barbershop"].getOverlays()
    end
    return {}
end
...
function getOverlayByIndex(index)
    ...
    elseif GetResourceState("nation_barbershop") == "started" then
        return exports["nation_barbershop"].getOverlayByIndex(index)
    end
end
...
AddEventHandler("spawn:setupChars", function()
   ...
   --COLOCAR EVENTO DA TELA DE LOGIN
    --TriggerEvent("zBank:open")
    toggleMultiChar()
end)
...
spawns = {
    ...
    { Name = "Garagem Sandy Shores", coords = {1624.05,3566.14,35.08} },
    { Name = "Garagem Paleto", coords = {-136.36,6357.03,31.49} },
...
function loginSpawn(status)
    ...
    primeirolog = primeirolog + 1
    if not status and primeirolog == 1 then
        if lastCoords then
	...
	if status then
        ...
        TriggerEvent('hud:Active', true)
        Wait(2000)
        TriggerEvent("registerPessoalInformation")
        TriggerEvent("Open:IngameWl")
	end
end
